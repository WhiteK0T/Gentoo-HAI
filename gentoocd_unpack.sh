...
if [[ $EUID -ne 0 ]]; then
  ...
  su -c "SET_PASS='${SET_PASS}' VPS_IP='${VPS_IP:-}' VPS_GW='${VPS_GW:-}' VPS_DNS='${VPS_DNS:-}' VPS_MTU='${VPS_MTU:-}' /bin/sh $0 ${ALLPOSITIONAL}" && [ "$AUTO" == "YES" ] && (rm -f kvm_lxgentootest.qcow2; sh test_w_qemu.sh -cdrom install-amd64-mod.iso ${POSITIONAL})
  exit
fi
...
for v in VPS_IP VPS_GW VPS_DNS VPS_MTU; do
  eval "_val=\${$v:-}"
